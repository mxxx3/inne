...
# --- TWARDY PATCH DLA WINDOWSA (nie przeszkadza na Linuxie) ---
def fixed_astimezone(obj): return pytz.utc
apscheduler.schedulers.base.astimezone = fixed_astimezone
os.environ['TZ'] = 'UTC'

# --- KONFIGURACJA POBIERANA Z SYSTEMU ---
# Token ustawisz w panelu Koyeb jako zmienną środowiskową
...
async def handle_bridge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id != GROUP_ID or thread_id not in BRIDGE:
        return
...
if __name__ == '__main__':
    if not TOKEN:
        ...
        application.add_handler(MessageHandler((filters.TEXT | filters.PHOTO) & (~filters.COMMAND), handle_bridge))
        print(f"Bot uruchomiony na serwerze! Mostek: Ogólny <-> {TOPIC_INNY}")
